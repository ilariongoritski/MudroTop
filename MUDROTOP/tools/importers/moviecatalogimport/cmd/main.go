...
func run(ctx context.Context) error {
	...
	for _, movie := range payload.Movies {
		...
		_, err := tx.Exec(ctx, `
			insert into movie_catalog_movies (
		...
		_, err = tx.Exec(ctx, `delete from movie_catalog_movie_genres where movie_id = $1`, movie.ID)
		if err != nil {
		...
		for _, genre := range movie.Genres {
			...
			_, err = tx.Exec(ctx, `
				insert into movie_catalog_genres (slug, label)
				values ($1, $2)
			...
			_, err = tx.Exec(ctx, `
				insert into movie_catalog_movie_genres (movie_id, genre_slug)
				values ($1, $2)
	...
	if len(importedIDs) == 0 {
		if _, err := tx.Exec(ctx, `delete from movie_catalog_movie_genres`); err != nil {
			return fmt.Errorf("clear movie genres: %w", err)
		}
		if _, err := tx.Exec(ctx, `delete from movie_catalog_genres`); err != nil {
			return fmt.Errorf("clear genres: %w", err)
		}
		if _, err := tx.Exec(ctx, `delete from movie_catalog_movies`); err != nil {
			return fmt.Errorf("clear movies: %w", err)
		}
	} else {
		if _, err := tx.Exec(ctx, `
			delete from movie_catalog_movies
		...
		if _, err := tx.Exec(ctx, `
			delete from movie_catalog_genres g
			where not exists (
				select 1
				from movie_catalog_movie_genres mg
