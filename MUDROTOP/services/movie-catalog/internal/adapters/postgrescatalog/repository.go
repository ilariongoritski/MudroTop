...
func (r *Repository) ListMovies(ctx context.Context, query domain.MovieQuery) (domain.MoviePage, error) {
	...
	rows, err := r.pool.Query(ctx, `
		...
		from movie_catalog_movies m
		left join movie_catalog_movie_genres mg on mg.movie_id = m.id
		left join movie_catalog_genres g on g.slug = mg.genre_slug
		where ($1::int is null or m.year >= $1)
		  and ($2::int is null or m.duration_minutes >= $2)
		  and ($3::text = '' or exists (
			select 1
			from movie_catalog_movie_genres mg2
			where mg2.movie_id = m.id and mg2.genre_slug = $3
		  ))
		  and (
			coalesce(cardinality($4::text[]), 0) = 0 or not exists (
				select 1
				from movie_catalog_movie_genres mg3
		...
		limit $5 offset $6
	`, query.YearMin, query.DurationMin, query.IncludeGenre, query.ExcludeGenres, query.PageSize, (query.Page-1)*query.PageSize)
	if err != nil {
		return domain.MoviePage{}, err
	}
	defer rows.Close()
...
func (r *Repository) countMovies(ctx context.Context, query domain.MovieQuery) (int, error) {
	...
	`, query.YearMin, query.DurationMin, query.IncludeGenre, query.ExcludeGenres).Scan(&total)
